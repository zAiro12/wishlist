<template>
  ...
  <div class="page-container">
    <div style="margin-bottom:1rem;"><RouterLink to="/groups">← Back to Groups</RouterLink></div>
    ...
    <template v-else-if="group">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="m in activeMembers" :key="m.id">
              ...
              <td v-if="isOwner">
                <button v-if="m.userId !== authStore.user?.id" class="remove-btn" @click="handleRemove(m)">Remove</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div style="display:flex;gap:0.75rem;">
        ...
        <button v-if="isOwner" class="btn-danger" style="padding:0.5rem 1rem;" @click="handleDeleteGroup">Delete Group</button>
      </div>
    </template>
  </div>
</template>
...
.birthday-banner { margin-bottom: 1.5rem; border-left: 4px solid var(--color-warning); }
.remove-btn { font-size: 0.75rem; padding: 0.2rem 0.5rem; background: var(--color-danger); color: white; border-radius: 4px; border: none; cursor: pointer; }
</style>
